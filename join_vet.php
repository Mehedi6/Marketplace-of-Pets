<html lang="en">
  ...
  <body> 
    ...
    <section id = "section2">
		<div class="title"> All Vets Qualifications List </div>
		<div style="margin-left:10%; margin-right:10%; margin-top:50px; margin-bottom:50px;text-align:center;background:#66b3ff;">
			<div class="row" style="padding:5px;"> 
				<div class="col-md-3">  Vet ID </div>
				<div class="col-md-3">  qualifications </div>
		...
		<?php 
			...
			$sql =  "SELECT  * from vet INNER JOIN vet_qualification on vet.id=vet_qualification.id";
		...
		<?php 
			require_once("DBconnect.php");
			$sql =  "SELECT  * from vet_qualification INNER JOIN vet on vet.id=vet_qualification.id";
			$result = mysqli_query($conn, $sql);
			if(mysqli_num_rows($result) > 0){
				//here we will print every row that is returned by our query $sql
				while($row = mysqli_fetch_array($result)){
