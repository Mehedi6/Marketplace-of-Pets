...
if(isset($_POST['sid']) && isset($_POST['sname']) && isset($_POST['cls']) && isset($_POST['address']) && isset($_POST['fees']) && isset($_POST['cat'])){
	...
	$result = mysqli_query($conn,"SELECT id FROM fosterhome WHERE id='$u'");
    $num_rows = mysqli_num_rows($result);

    if ($num_rows) {
        echo ("<script LANGUAGE='JavaScript'>
             window.alert('That ID is already taken. Please enter a new id');
             window.location.href='add_fosterhomes.php';
          </script>");
	...
	$sql = " INSERT INTO fosterhome VALUES( '$u', '$p', '$c', '$a' , '$x' , '$y' ) ";
	...
	//if(mysqli_affected_rows($conn)){
		...
		header("Location: show_fosterhomes.php");
	//}
	//else{
